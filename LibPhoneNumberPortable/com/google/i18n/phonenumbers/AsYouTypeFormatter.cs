...
using NumberFormat = com.google.i18n.phonenumbers.nano.Phonemetadata.NumberFormat;
using PhoneMetadata = com.google.i18n.phonenumbers.nanoPhonemetadata.PhoneMetadata;
...
public class AsYouTypeFormatter {
  ...
  private static readonly Pattern CHARACTER_CLASS_PATTERN = Pattern.compile("\\[([^\\[\\]])*\\]");
  ...
  private static readonly Pattern STANDALONE_DIGIT_PATTERN = Pattern.compile("\\d(?=[^,}][^,}])");
  ...
  // output of the AYTF.
  private static readonly Pattern ELIGIBLE_FORMAT_PATTERN =
      Pattern.compile("[" + PhoneNumberUtil.VALID_PUNCTUATION + "]*" +
          "(\\$\\d" + "[" + PhoneNumberUtil.VALID_PUNCTUATION + "]*)+");
  // A set of characters that, if found in a national prefix formatting rules, are an indicator to
  // us that we should separate the national prefix from the number when formatting.
  private static readonly Pattern NATIONAL_PREFIX_SEPARATORS_PATTERN = Pattern.compile("[- ]");
  ...
  private static readonly Pattern DIGIT_PATTERN = Pattern.compile(DIGIT_PLACEHOLDER);
